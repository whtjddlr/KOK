...
export function InvitePage() {
  ...
  return (
    <main className="min-h-screen bg-[#f8fbf7] text-[#17233c]">
      <section className="mx-auto flex min-h-screen w-full max-w-md flex-col px-6 py-8">
        <header className="flex items-center justify-between">
          <a href="/landing" className="flex items-center gap-3" aria-label="KoK 소개로 이동">
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-[#ffd9d9] shadow-[0_10px_24px_rgba(255,123,107,0.2)]">
        ...
        <div className="flex flex-1 flex-col justify-center py-10">
          ...
          <h1 className="text-4xl font-black leading-tight tracking-normal text-[#17233c]">
            같은 약속방에서
            <br />
            장소를 정해요.
          </h1>
          ...
          <div className="mt-9 rounded-[1.75rem] border border-[#e6ece8] bg-white p-5 shadow-[0_16px_38px_rgba(31,42,68,0.08)]">
            <div className="text-sm font-semibold text-[#7a8684]">초대 코드</div>
            <div className="mt-2 rounded-2xl bg-[#f5f8f4] px-4 py-4 text-center font-mono text-3xl font-black tracking-wide text-[#17233c]">
              {roomCode || '준비 중'}
            </div>
            {!roomCode ? (
          ...
          <div className="mt-6 grid gap-3">
            <a
              href={joinUrl}
              className="inline-flex h-14 items-center justify-center gap-2 rounded-2xl bg-[#17233c] px-5 text-base font-black text-white shadow-[0_14px_30px_rgba(23,35,60,0.18)] transition-transform active:scale-[0.98]"
            >
              약속방 참여하기
            ...
            <button
              type="button"
              onClick={handleCopy}
              className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl border border-[#e5e9ec] bg-white px-5 text-sm font-bold text-[#17233c] shadow-sm transition-transform active:scale-[0.98]"
            >
              {copied ? (
